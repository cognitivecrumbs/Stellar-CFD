...
void CFDMeshWrapper::update_mesh() {
    float dt = 0.0011/2; // Use a fixed timestep
...
void CFDMesh::calcSlope() {
    // Compute slopes for the x-direction
    this->slope_x = (this->neighbor[2]->U - this->neighbor[0]->U) / (2*this->dx);

    // Compute slopes for the y-direction
    this->slope_y = (this->neighbor[3]->U - this->neighbor[1]->U) / (2*this->dx);
}
...
void CFDMesh::computeHLLFlux() {
    ...
    for (size_t i = 0; i < this->neighbor.size(); ++i) {
        ...
        switch (i) {
        case 0: // Left face
            U_L = this->U + this->slope_x * dx_half;
            U_R = neighbor->U - neighbor->slope_x * dx_half;
            normal_x = 1;
            break;
        case 1: // Bottom face
            U_L = this->U + this->slope_y * dx_half;
            U_R = neighbor->U - neighbor->slope_y * dx_half;
            normal_y = 1;
            break;
        case 2: // Right face
            U_L = neighbor->U + neighbor->slope_x * dx_half;
            U_R = this->U - this->slope_x * dx_half;
            normal_x = -1;
            break;
        case 3: // Top face
            U_L = neighbor->U + neighbor->slope_y * dx_half;
            U_R = this->U - this->slope_y * dx_half;
            normal_y = -1;
            break;
        }
    ...
    this->dUdt /= this->vol; // Normalize by volume.
    this->bodyForce();
}
